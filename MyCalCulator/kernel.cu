...
extern "C" __declspec(dllexport) void exportCppFunctionSub(int *src, int* src2, int* dst, int arraySize);
...
extern "C" __declspec(dllexport) void exportCppFunctionAdd(int *src, int* src2, int* dst, int arraySize) {
	...
	if (cudaStatus != cudaSuccess)
		...
		printf("exportCppFuncationAdd Success\n");
}
...
int main()
{
	...
	int device_count = 0;
	cudaError_t deviceCountError = cudaGetDeviceCount(&device_count);
	...
	cudaError_t deviceError = cudaGetDeviceProperties(&deviceProp, 0);
...
extern "C" __declspec(dllexport) void exportCppFunctionSub(int *src, int* src2, int* dst, int arraySize) {
	...
	cudaError_t cudaStatus = subWithCuda(dst, src, src2, arraySize);

	if (cudaStatus != cudaSuccess)
		fprintf(stderr, "sunWithCuda failed!");
	else
		printf("exportCppFuncationAdd Success\n");
}
